...
See also `roll`.
"""
function rollp(dim::Integer, fn::F, Xₜ::AbstractArray, w::Integer, val::M=missing) where {F<:Function,M}
...
Using this instead of `rollp(1, ...)` will be marginally more performant.
"""
function rollp(fn::F, Xₜ::AbstractArray, w::Integer, val::M=missing) where {F<:Function,M}
	out = roll(fn, Xₜ, w)
